...
from samplers import fm_euler_sampler

from dataset import ImgVAELabelDataset
...
def main(args):
    ...
    model = SiT_models[args.model](
        ...
        use_cfg_embedding = (args.cfg_prob > 0),
        repa_depth=8 if args.proj_coeff > 0. else 0,
        z_dims = [768],
        **block_kwargs
    )
    if args.pretrained:
        ...
        if args.legacy:
            ...
            ckpt = torch.load(args.pretrained, map_location="cpu", weights_only=False)["model"]
    ...
    if accelerator.is_main_process:
        logger.info(f"Training Parameters: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
    ...
    # Setup data:
    train_dataset = ImgVAELabelDataset(args.data_dir)
    ...
    if args.resume_step > 0:
        ...
        if accelerator.is_main_process:
            logger.info(f"Resumed from {global_step}, {msg}")
    ...
    for epoch in range(args.epochs):
        model.train()
        for img, x, y in train_dataloader:
            ...
            if args.proj_coeff > 0.:
                ...
                with accelerator.autocast():
                    zs.append(encoder.forward_features(img)['x_norm_patchtokens'])
            else:
                zs = None
            with accelerator.autocast():
                loss, loss_dict = loss_fn(compiled_model, x, y, zs=zs)
            loss = loss.mean()
            ...
            if (global_step == 100 or (global_step % args.sampling_steps == 0 and global_step > 0)):
                with torch.no_grad():
                    with accelerator.autocast():
                        samples = fm_euler_sampler(
                            ema, xT, ys, **sampling_kwargs
                        ).to(torch.float32)
...
def parse_args(input_args=None):
    ...
    #training loss
    parser.add_argument("--loss-type", type=str, default="dmf", choices=["dmf", "dmf_logvar", "mf", "si", "repa"])
    parser.add_argument("--P-mean", type=float, default=-0.4, help="Mean of the P distribution.")
    ...
    parser.add_argument("--ema-decay", type=float, default=0.9999, help="Exponential moving average decay rate.")
